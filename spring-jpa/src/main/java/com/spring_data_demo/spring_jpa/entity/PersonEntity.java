...
/*
 * @Table and @Entity annotations trigger hibernate's feature called schema update. This is due to Springboot's auto configuration
 * and happens because we are using in-memory database.
 * Hence, no need to create table explicitly using .sql file in resources.
 */
// @NamedQuery annotation is used to define java persistence query langauge query
// the query below selects all objects of type PersonEntity
// JPQL uses entity name instead of table name for queries.
@NamedQuery(name = "show_all_persons", query = "select p from PersonEntity p")
